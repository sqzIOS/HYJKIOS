...
+ (HBButton *)constructHBButton:(CGRect)frame
                        ...
                        slBgImg:(UIImage *)slBgImg
                         ...
                         action:(SEL)action
                       userInfo:(id)userInfo {
    HBButton *btn = [self constructHBButton:frame
                                   target:target
                                   action:action
                                   userInfo:userInfo];
    if (nlBgImg) {
...
+ (UIImageView *)constructHBImgViewIpad:(NSString *)imgName{
    UIImage *image = [UIImage imageNamed:imgName];
    image = [ImageUtil zoomToSize:image size:HBSizeIpadOfSize(image.size)];
...
+ (HBImageView *)constructHBImageView:(CGRect)frame
                              imgName:(NSString *)imgName
                             userInfo:(id)userInfo{
    HBImageView *result = [[HBImageView alloc] initWithFrame:frame];
    result.userInteractionEnabled = NO;
    UIImage *image = [UIImage imageNamed:imgName];
    result.image = image;
