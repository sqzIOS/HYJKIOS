...
////    NSString *urlEncode = [StringUtil urlEncode:base64Str];  by:zxwo0o 重复encode ASI会加密一次
...
+(UIImage *)defaultImageWithSize:(CGSize)size centerImage:(UIImage *)centerImage{
    ...
    UIGraphicsBeginImageContext(rect.size);
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextSetFillColorWithColor(context, [RGBCOLOR(241, 241, 241) CGColor]);
    CGContextFillRect(context, rect);
    [centerImage drawInRect:CGRectMake((size.width - centerImage.size.width)/2, (size.height - centerImage.size.height)/2, centerImage.size.width, centerImage.size.height)];
...
//        return [self defaultImageWithSize:size centerImage:DefaultImage_180];
    }else{
//        return [self defaultImageWithSize:size centerImage:DefaultImage_110];
    }
    return nil;
}
...
+(UIImage *)defaultImageWithView:(UIView *)view{
    return [self defaultImageWithSize:view.frame.size];
}
