...
//        [[instance imageView] stopAnimating];
//        [[instance imageView] setImage:[UIImage animatedImageWithAnimatedGIFURL:[[NSBundle mainBundle] URLForResource:@"爱心.gif" withExtension:nil]]];
    }
    return instance;
}
...
//        [self.layer setBackgroundColor:[[UIColor colorWithWhite:0 alpha:0.5] CGColor]];
        [self.layer setBackgroundColor:[[UIColor colorWithWhite:1 alpha:0.9] CGColor]];
...
+ (void)showWithOverlay {
    [self dismiss:^{
        [APPDELEGATE.window addSubview:[[self instance] overlaySelf]];
        [self show];
    }];
}
...
+ (void)dismiss {
    ...
    [[[self instance] overlaySelf] removeFromSuperview];
