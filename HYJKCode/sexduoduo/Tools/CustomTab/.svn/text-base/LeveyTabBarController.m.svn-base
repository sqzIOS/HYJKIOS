...
//		_transitionView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320.0f, _containerView.bounds.size.height - kTabBarHeight)];
        
        _transitionView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, _containerView.bounds.size.height )];
...
- (void)displayViewAtIndex:(NSUInteger)index
{
    ...
    NSString *notifiStr = [NSString stringWithFormat:@"displayView%d",(int)index];
    
    [[NSNotificationCenter defaultCenter] postNotificationName:notifiStr object:nil];
	...
	if ([selectedVC.view isDescendantOfView:_transitionView]) 
	{
        ...
        [rootVc.view addSubview:_tabBar];
	}
	else
	{
