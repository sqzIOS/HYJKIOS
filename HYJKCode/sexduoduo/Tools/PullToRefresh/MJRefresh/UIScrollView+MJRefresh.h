...
/**
 *  设置头部控件的文字
 类目无法添加实例变量 但是可以添加不生成实例变量的属性，并且自己写setter 和 getter方法用来给self的某个已经有的实例变量赋值 例如frame之类的 就像UIView+MJExtension中的一样
 注意：但是却可以通过runtime的方法模拟添加属性
修改的地方：
 1.本类中.h 属性 .m中set 和 get方法将本类目扩的属性赋值到一个 实体header或foot类里 本类应该也可以保存
 2.header 和 foot的基类 MJRefreshBaseView 也添加属性（因为类目没有位置添加实例变量所以属性不能自动生成实例变量用来修改但是我们可以修改实体类的 实例变量）
 3.MJRefreshFooterView 类中的layoutSubviews 重写 箭头位置
 4.MJRefreshHeaderView  类中的 layoutSubviews 重写 header的 箭头位置
...
@property (assign,nonatomic)BOOL isFenLei;//分类使用 单独修改一下布局
