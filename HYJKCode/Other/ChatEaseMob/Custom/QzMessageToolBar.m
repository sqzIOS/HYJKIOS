...
@interface  QzMessageToolBar ()<UITextViewDelegate>
{
...
@property (strong, nonatomic) UIButton *moreButton;//发送按钮
...
@implementation QzMessageToolBar
...
- (void)setupSubviews
{
    ...
    self.moreButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) - kHorizontalPadding - kInputTextViewMinHeight*1.5, kVerticalPadding, kInputTextViewMinHeight*1.5, kInputTextViewMinHeight)];
    [self.moreButton setTitle:@"发送" forState:UIControlStateNormal];
    [self.moreButton setTitleColor:colorWith01 forState:UIControlStateNormal];
    [self.moreButton setBackgroundColor:WHITE_COLOR];
    [self.moreButton addTarget:self action:@selector(sendMessage) forControlEvents:UIControlEventTouchUpInside];
    [self.moreButton qzSetBorderColor:[UIColor colorWithWhite:0.8 alpha:1.0]];
    [self.moreButton qzSetCornerRadius:6.0];
    [self.moreButton qzSetBorderWidth:0.65];
    self.moreButton.tag = 2;
    allButtonWidth += CGRectGetWidth(self.moreButton.frame) + kHorizontalPadding * 2.5;
    
    self.senderBtn=self.moreButton;
