...
-(void)defaultSettings
{
    ...
    titleLabel.textColor = colorWith01;
    [btn addSubview:titleLabel];
    
    UIImage *image = [UIImage imageNamed:_arrowImgName];
    _arrow = [[UIImageView alloc]initWithFrame:CGRectMake(btn.width - image.size.width - APP_SPACE(2), btn.height - image.size.height - APP_SPACE(2), image.size.width, image.size.height)];
    _arrow.image = image;
