...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        imageViewBg.image = [UIImage imageNamed:@"record_diary_love"];
...
- (void)configureCell
{
    ...
    _imageViewBg.frame = CGRectMake(self.contentView.fs_width/6, 0, self.contentView.fs_width*2/3, self.contentView.fs_height*2/3);
    _imageViewBg.hidden = !_hasEvent;
    ...
    CGFloat titleHeight;
    if (IOS7) {
        titleHeight = [_titleLabel.text sizeWithAttributes:@{NSFontAttributeName:self.titleLabel.font}].height;
    } else {
        titleHeight = [_titleLabel.text sizeWithFont:_titleLabel.font].height;
    }
    if (_subtitleLabel.text) {
        _subtitleLabel.hidden = NO;
        CGFloat subtitleHeight;
        if (IOS7) {
            subtitleHeight = [_subtitleLabel.text sizeWithAttributes:@{NSFontAttributeName:self.subtitleLabel.font}].height;
        } else {
            subtitleHeight = [_titleLabel.text sizeWithFont:_titleLabel.font].height;
        }
